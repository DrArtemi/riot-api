<script>
	...
	import { TEAM_MATCHES, TOURNAMENTS_BY_SLUGS } from '../components/queries';
	...
	$: matches.refetch({ team: slug });
	matches.subscribe((result) => {
		if (result.data) {
			tournament_slugs = [];
			for (const match of result.data.teamMatches)
...
<div class="pt-24 pb-2 px-4 h-full w-full flex flex-col">
	<div class="flex-initial w-full bg-slate-800/30 border-l-2 border-l-amber-400">
		<MatchFilters {tournaments} {tournaments_stages} bind:tournaments_filters bind:stages_filters />
	</div>
	<div class="flex flex-row flex-1 min-h-0 mt-2 border-l-2 border-l-amber-400">
		<div class="w-64 bg-slate-800/30 flex-initial">
			<SideBar title={slug} {tabs} bind:activeTab />
		</div>
		<div class="w-full h-full ml-2 p-8 bg-slate-800/30 flex flex-col flex-1">
			{#if tabs[activeTab] == 'matches'}
				...
				<div class="text-slate-200 overflow-auto flex-1">
					{#if $matches.data}
						<MatchList {tournaments_filters} {stages_filters} matches={$matches.data.teamMatches} />
					{:else}
						<p>No match found</p>
					{/if}
				</div>
